...
// Глобальное определение типа устройства
...
function init() {
    ...
    camera = new THREE.PerspectiveCamera(
        ...
        0.1,
        isMobile ? 200 : 1000          // Меньше дальность на мобильном
    );
...
function startGame() {
    ...
    if (isTouch) {
        // На мобильном pointerlock не нужен
        isLocked = true;
    } else {
...
function createOtherPlayer(playerData) {
    // Контейнер — используем сразу, модель добавится асинхронно
    const container = new THREE.Group();
    container.position.set(playerData.x, playerData.y - 1.8, playerData.z);
    ...
    hpBar.position.set(-0.0, 2.1, 0.01);
    container.add(hpBar);
    container.userData.hpBar = hpBar;
    ...
    loadPlayerModel((model) => {
        // Цвет команды поверх материалов
        ...
        model.traverse(child => {
            if (child.isMesh) {
                // Тонируем модель цветом команды слабо
...
function setupMobileControls() {
    ...
    btnShoot.addEventListener('touchstart', (e) => {
        e.preventDefault();
        isLocked = true; // разрешаем стрельбу без pointerlock на мобильном
        shoot();
    }, { passive: false });
    ...
    // На мобильном isLocked уже установлен в startGame
}
